...
{
    Istream& is = this->readStream(typeName);
    ...
    {
        DimensionedField<Type, GeoMesh>::operator+=(fieldAverage);
...
template<class Type, template<class> class PatchField, class GeoMesh>
Foam::DimensionedField<Type, GeoMesh>&
...
{
    Foam::max(internalField(), internalField(), dt);
...
{
    Foam::min(internalField(), internalField(), dt);
...
)
{
    ...
    (
        const_cast<DimensionedField<Type, GeoMesh>&>(gf.internalField())
    );
