...
)
{
    ...
    do
    {
        ...
        if (routeFaceI == -1)
        {
            Info<< "No faces left to route. Either all scheduled or all"
                << " its cells already busy." << endl;
        ...
        Info<< "Making connection across face " << routeFaceI
            << " between " << myCells[0] << " and " << myCells[1] << endl;
...
)
{
    ...
    do
    {
        Info<< "Starting iteration " << commIter << endl << endl;
...
// Construct from processor-processor topology
Foam::commSchedule::commSchedule(const processorTopology& topo)
:
    labelListList(topo.cellFaces().size()),
    faceSchedule_(topo.faceCells().size(), -1)
{
    scheduleAll(topo.cellFaces(), topo.faceCells());
}
