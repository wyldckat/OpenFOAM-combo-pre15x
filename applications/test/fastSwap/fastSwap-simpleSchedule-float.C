...
//        << " bytes" << endl;
    ...
    if (blockingRecv)
    {
        ...
        if (messageSize > (recvBuf.size() * sizeof(float)))
        {
...
#   include "createFields.H"
    ...
    labelList procPatchNo(patches.size());
    ...
    forAll(patches, patchI)
    {
        ...
        if (patch.type() == processorPolyPatch::typeName)
        {
            ...
            procPatchNo[nProcPatches] = nProcPatches;
    ...
    List<List<float> > sendBuf(nProcPatches);
    List<List<float> > recvBuf(nProcPatches);
    ...
    for (label iter = 0; iter < nSwaps; iter++)
    {
        ...
        forAll(procPatches, procPatchI)
        {
            ...
            if (!blockingRecv)
            {
                ...
                if (messageSize > (recvBuf[procPatchI].size() * sizeof(float)))
                {
                    FatalErrorIn("IPstream::IPstream(const int fromProcNo)")
    ...
    Info<< "Done " << endl
        ...
        << "    blockingSend:" << blockingSend << endl
        << "    blockingRecv:" << blockingRecv << endl
        << "    user time   :" << runTime.cpuTimeIncrement() << endl
        << "    wall time   :" << wallTimer.elapsedClockTime() << endl
        << endl << endl;
