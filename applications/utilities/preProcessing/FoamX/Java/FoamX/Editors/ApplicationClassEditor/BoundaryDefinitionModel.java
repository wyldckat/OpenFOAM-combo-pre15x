    ...
    implements FieldModelListener
{
    ...
    private WeakReference appClassModel_;         // Weak reference to the application class model object.
    ...
    BoundaryDefinitionModel(ApplicationClassModel appClassModel)
    {
        try
        {
            appClassModel_         = new java.lang.ref.WeakReference(appClassModel);
    ...
    DefaultMutableTreeNode addBoundaryDefinitionNode(String boundaryTypeName, DefaultMutableTreeNode parentNode)
    {
        DefaultMutableTreeNode newNode = null;

        try
        {
            if (!boundaryDefMap_.containsKey(boundaryTypeName))
            {
                // Create a new Boundary Type Descriptor object.
                IBoundaryTypeDescriptorHolder bndTypeDescHolder = new IBoundaryTypeDescriptorHolder();
                getAppClassModel().getAppClassDescriptor().addBoundaryType(boundaryTypeName, bndTypeDescHolder);
    ...
    boolean deleteBoundaryDefinitionNode(DefaultMutableTreeNode nodeItem)
    {
        ...
        try
        {
            ...
            if (nodeItem.getChildCount() == 0 && nodeInfo.isBoundaryDefinition())
            {
                ...
                getAppClassModel().getAppClassDescriptor().deleteBoundaryType(nodeInfo.getName());
    ...
    void updateApplicationClass(IApplicationClass appClass)
    {
        try
        {
            // Update the boundary type desriptor objects.
            ...
            while (enum.hasMoreElements())
            {
                DefaultMutableTreeNode      node     = (DefaultMutableTreeNode)enum.nextElement();
                BoundaryDefinitionModelItem nodeInfo = (BoundaryDefinitionModelItem)node.getUserObject();
                nodeInfo.updateBoundaryTypeDescriptor();
            }
        }
        catch (Exception ex)
        {
    ...
    private ApplicationClassModel getAppClassModel()
    {
        return (ApplicationClassModel)appClassModel_.get();
    }
    ...
    private void initialiseBoundaryTypes()
    {
        try
        {
            ...
            String[] boundaryTypes = getAppClassModel().getAppClassDescriptor().boundaryTypes();
            for (int i=0; i <boundaryTypes.length; i++)
            {
                // Get Boundary Type Descriptor object.
                IBoundaryTypeDescriptorHolder bndTypeDescHolder = new IBoundaryTypeDescriptorHolder();
                getAppClassModel().getAppClassDescriptor().getBoundaryType(boundaryTypes[i], bndTypeDescHolder);
                ...
                {
                    throw new FoamXException("Duplicate boundary definition in BoundaryDefinitionModel::initialiseBoundaryTypes.");
                }
                ...
                // Add to map.
                boundaryDefMap_.put(boundaryTypes[i], newNode);
            }
            ...
            while (enum.hasMoreElements())
            {
                ...
                {
                    throw new FoamXException("Unknown patch type in BoundaryDefinitionModel::initialiseBoundaryTypes.");
                }

                // If we have a super type, make sure we have it in the map.
                if (nodeInfo.getSuperType().length()> 0 && !boundaryDefMap_.containsKey(nodeInfo.getSuperType()))
                {
                    throw new FoamXException("Invalid super type in boundary definition in BoundaryDefinitionModel::initialiseBoundaryTypes.");
                }

                // Get the parent node.
                DefaultMutableTreeNode parentNode = null;
                if (nodeInfo.getSuperType().length()> 0)
                {
                    // Sub-type boundary definition.
                    parentNode = (DefaultMutableTreeNode)boundaryDefMap_.get(nodeInfo.getSuperType());
                }
                else
                {
