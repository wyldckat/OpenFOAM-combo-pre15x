    ...
    extends javax.swing.JPanel
{
    ...
    //--------------------------------------------------------------------------
    /** Constructor for BoundaryTypePanel. */
    public FieldsTab(ApplicationClassModel appClassModel)
    {
        try
        {
            appClassModel_ = appClassModel;
            ...
            int height = (table_.getRowHeight() + table_.getRowMargin()) *
                         (appClassModel_.getFieldDefinitionModel().getRowCount() + 1);    // Add an extra row to account for the header control.
            ...
            String[] foamTypes = appClassModel_.getFoamTypes();
            for (int i = 0; i <foamTypes.length; i++)
            {
                ITypeDescriptor fieldType = appClassModel_.getFoamType(foamTypes[i]);
            ...
            String[] geomTypes = appClassModel_.getGeometryTypes();
            for (int i = 0; i <geomTypes.length; i++)
            {
                IGeometryDescriptor geomType = appClassModel_.getGeometryType(geomTypes[i]);
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        table_.setModel( appClassModel_.getFieldDefinitionModel() );
    ...
    extends AbstractAction
    {
        ...
        public void actionPerformed(ActionEvent e)
        {
            try
            {
                ...
                appClassModel_.getFieldDefinitionModel().addNewField();
            }
            catch (Exception ex)
            {
                App.handleAllExceptions(ex);
        ...
        public void actionPerformed(ActionEvent e)
        {
            ...
            try
            {
                ...
                if (!table_.getSelectionModel().isSelectionEmpty())
                {
                    ...
                    {
                        appClassModel_.getFieldDefinitionModel().removeField(i);
                    }
