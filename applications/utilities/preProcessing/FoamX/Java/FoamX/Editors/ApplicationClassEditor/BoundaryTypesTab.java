    ...
    extends javax.swing.JPanel
{
    ...
    private AddBoundaryTypeAction addBoundaryTypeAction_;
    private DeleteBoundaryTypeAction deleteBoundaryTypeAction_;

    //--------------------------------------------------------------------------
    /** Constructor for DictionaryPanel. */
    public BoundaryTypesTab(ApplicationClassModel appClassModel)
    {
        try
        {
            boundaryDefinitionModel_ = appClassModel.getBoundaryDefinitionModel();
            boundaryDefinitionPanel_ = new BoundaryDefinitionPanel(appClassModel);
            ...
            addBoundaryTypeAction_    = new AddBoundaryTypeAction();
            deleteBoundaryTypeAction_ = new DeleteBoundaryTypeAction();
            addToolbarButton(addBoundaryTypeAction_);
            addToolbarButton(deleteBoundaryTypeAction_);

            // Initialise actions
            addBoundaryTypeAction_.setEnabled(false);
            deleteBoundaryTypeAction_.setEnabled(false);
        }
        catch (Exception ex)
        {
  ...
  private void OnTreeMouseClicked (java.awt.event.MouseEvent evt)
        ...
        try
        {
            ...
            if (tp != null)
            {
                ...
                if (evt.getModifiers() == evt.BUTTON3_MASK)   // Check explicitly for right mouse click.
                {
                    ...
                    {
                        contextMenu_.add(addBoundaryTypeAction_).setFont(font);
                    }
                    else if (nodeInfo.isBoundaryDefinition())
                    {
                        contextMenu_.add(addBoundaryTypeAction_).setFont(font);
                        contextMenu_.add(deleteBoundaryTypeAction_).setFont(font);
                    }
        ...
        try
        {
            ...
            if (evt.isAddedPath())    // Has the new node been added to the selection?
            {
                ...
                if (tp != null)
                {
                    ...
                    addBoundaryTypeAction_.setEnabled(nodeInfo.isPatchType() || nodeInfo.isBoundaryDefinition());
                    deleteBoundaryTypeAction_.setEnabled(nodeInfo.isBoundaryDefinition() && nodeItem.getChildCount() == 0); // Can't delete anything with a child, yet.
    ...
    class AddBoundaryTypeAction
    extends AbstractAction
    {
        AddBoundaryTypeAction()
        {
    ...
    class DeleteBoundaryTypeAction
    extends AbstractAction
    {
        DeleteBoundaryTypeAction()
        {
            putValue(Action.SMALL_ICON, App.getResources().getIcon("DelBoundaryTypeImage"));
