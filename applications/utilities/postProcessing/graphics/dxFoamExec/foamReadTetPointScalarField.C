...
)
{
    try
    {
        tetPointScalarField psf(fieldObject, mesh.tetMesh());

        // Use only points corresponding to polyMesh vertices
        // (first points in psf)
        float *pointScalarArray = new float[mesh.totalNPoints()];

        for(label i = 0; i < mesh.totalNPoints(); i++)
        {
        ...
        )
        {
            ...
            (
                ERROR_INTERNAL,
                "dxFoamExec:foamReadTetPointScalarField : Error in DXAddArrayData"
            );
