...
Error m_foamGetTimes(Object *in, Object *out)
{
    ...
    if (times.size() > 0)
    {
        ...
        const char **timeNameStrings = new const char*[nTimes];

        forAll(times, i)
        {
            timeNameStrings[i] = times[i].name().c_str();
            timeValues[i] = times[i].value();
        }

        dxTimeNames = DXMakeStringListV(nTimes, (char**)timeNameStrings);
...
Error m_foamGetFieldNames(Object *in, Object *out)
{
    ...
    )
    {
        ...
        if (nFields)
        {
            const char** fieldNameStrings = new const char*[nFields];

            forAll(fieldNames, i)
            {
                fieldNameStrings[i] = fieldNames[i].c_str();
            }
