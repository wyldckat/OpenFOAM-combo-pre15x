...
)
{
    try
    {
        GeometricField<Type, tetPolyPatchField, tetPointMesh> ptf
        (
            fieldObject,
            mesh.tetMesh()
        );
        ...
        label i=0;
        for (label verti = 0; verti < mesh.totalNPoints(); verti++)
        {
        ...
        )
        {
            ...
            (
                ERROR_INTERNAL,
                "dxFoamExec:foamReadTetPointField : Error in DXAddArrayData"
            );
