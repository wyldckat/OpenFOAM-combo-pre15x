...
#include "directMeshCutAndRemove.H"
#include "polyMesh.H"
...
defineTypeNameAndDebug(directMeshCutAndRemove, 0);
...
Foam::label Foam::directMeshCutAndRemove::firstCommon
(
...
bool Foam::directMeshCutAndRemove::isIn
(
    const edge& twoCuts,
...
Foam::label Foam::directMeshCutAndRemove::findCutCell
(
    const cellCuts& cuts,
...
//  none found.
Foam::label Foam::directMeshCutAndRemove::findInternalFacePoint
(
...
) const
{
    ...
    {
        FatalErrorIn("directMeshCutAndRemove::findInternalFacePoint(const labelList&)")
...
// to the patch
Foam::label Foam::directMeshCutAndRemove::findPatchFacePoint
(
    const face& f,
...
void Foam::directMeshCutAndRemove::faceCells
(
    const cellCuts& cuts,
    const label exposedPatchI,
    const label faceI,
...
void Foam::directMeshCutAndRemove::getZoneInfo
(
    const label faceI,
    label& zoneID,
    bool& zoneFlip
...
void Foam::directMeshCutAndRemove::addFace
(
...
void Foam::directMeshCutAndRemove::modFace
(
    directPolyTopoChange& meshMod,
    const label faceI,
    const face& newFace,
...
void Foam::directMeshCutAndRemove::copyFace
(
    const face& f,
    const label startFp,
    const label endFp,
...
// (addedPoints_).
void Foam::directMeshCutAndRemove::splitFace
(
    const face& f,
    const label v0,
    const label v1,
...
) const
{
    ...
    if (startFp == -1)
    {
        FatalErrorIn
        (
            "directMeshCutAndRemove::splitFace"
...
Foam::face Foam::directMeshCutAndRemove::addEdgeCutsToFace(const label faceI) const
{
    const face& f = mesh().faces()[faceI];
...
Foam::face Foam::directMeshCutAndRemove::loopToFace
(
    const label cellI,
...
Foam::directMeshCutAndRemove::directMeshCutAndRemove(const polyMesh& mesh)
:
    edgeVertex(mesh),
    addedFaces_(),
    addedPoints_()
...
void Foam::directMeshCutAndRemove::setRefinement
(
    const label exposedPatchI,
    const cellCuts& cuts,
...
)
{
    ...
    if (exposedPatchI < 0 || exposedPatchI >= patches.size())
    {
        FatalErrorIn
        (
            "directMeshCutAndRemove::setRefinement("
    ...
    if (debug)
    {
        Pout<< "directMeshCutAndRemove:" << nl
...
void Foam::directMeshCutAndRemove::updateMesh(const mapPolyMesh& map)
{
    // Update stored labels for mesh change.
    {
        ...
        )
        {
            ...
            if ((newCellI != -1) && (newAddedFaceI != -1))
            {
                ...
                {
                    Pout<< "directMeshCutAndRemove::updateMesh :"
