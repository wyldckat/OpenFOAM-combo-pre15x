...
Foam::label Foam::directEdgeCollapser::findIndex
(
...
Foam::label Foam::directEdgeCollapser::changePointRegion
(
    const label pointI,
...
bool Foam::directEdgeCollapser::pointRemoved(const label pointI) const
{
...
void Foam::directEdgeCollapser::filterFace(const label faceI, face& f) const
{
    label newFp = 0;
    ...
    for (label fp = 2; fp < size; fp++)
    {
        ...
        if (index == fp1)
        {
            WarningIn
            (
                "Foam::directEdgeCollapser::filterFace(const label faceI, "
                "face& f) const"
...
// Debugging.
void Foam::directEdgeCollapser::printRegions() const
{
...
void Foam::directEdgeCollapser::collapseEdges(const labelList& edgeLabels)
{
...
// Construct from mesh
Foam::directEdgeCollapser::directEdgeCollapser(const polyMesh& mesh)
:
    mesh_(mesh),
...
bool Foam::directEdgeCollapser::unaffectedEdge(const label edgeI) const
{
...
bool Foam::directEdgeCollapser::collapseEdge(const label edgeI, const label master)
{
    const edge& e = mesh_.edges()[edgeI];
    ...
    if (pointRegion0 == -1)
    {
        if (pointRegion1 == -1)
        {
            ...
            if (freeRegions_.size() > 0)
            {
                ...
                {
                    FatalErrorIn
                    ("directEdgeCollapser::collapseEdge(const label, const label)")
...
bool Foam::directEdgeCollapser::setRefinement(directPolyTopoChange& meshMod)
{
