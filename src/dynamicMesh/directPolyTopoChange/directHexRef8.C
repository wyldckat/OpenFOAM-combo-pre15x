...
namespace Foam
{
    defineTypeNameAndDebug(directHexRef8, 0);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::directHexRef8::reorder
(
    const labelList& map,
    const label len,
    const label null,
...
)
{
    ...
    forAll(elems, i)
    {
        ...
        if (newI >= len)
        {
            FatalErrorIn("directHexRef8::reorder") << abort(FatalError);
        }
...
void Foam::directHexRef8::getFaceInfo
(
    const label faceI,
    label& patchID,
    label& zoneID,
...
Foam::label Foam::directHexRef8::addFace
(
...
// for the face)
Foam::label Foam::directHexRef8::addInternalFace
(
    directPolyTopoChange& meshMod,
...
void Foam::directHexRef8::modFace
(
    directPolyTopoChange& meshMod,
    const label faceI,
    const face& newFace,
...
Foam::label Foam::directHexRef8::getAnchorCell
(
...
) const
{
    if (cellAnchorPoints[cellI].size() > 0)
    {
        ...
        FatalErrorIn("directHexRef8::getAnchorCell")
...
void Foam::directHexRef8::getFaceNeighbours
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const label faceI,
...
Foam::label Foam::directHexRef8::findMinLevel(const labelList& f) const
{
...
Foam::label Foam::directHexRef8::findMaxLevel(const labelList& f) const
{
...
Foam::label Foam::directHexRef8::countAnchors
(
    const labelList& f,
...
Foam::label Foam::directHexRef8::findLevel
(
    const face& f,
    const label startFp,
...
) const
{
    ...
    forAll(f, i)
    {
        ...
        {
            FatalErrorIn("directHexRef8::findLevel")
                << "face:" << f
...
Foam::label Foam::directHexRef8::getAnchorLevel(const label faceI) const
{
    const face& f = mesh_.faces()[faceI];
    ...
    else
    {
        ...
        {
            FatalErrorIn("directHexRef8::getAnchorLevel(const label) const")
                << "face:" << faceI
...
void Foam::directHexRef8::checkInternalOrientation
(
    directPolyTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
void Foam::directHexRef8::checkBoundaryOrientation
(
    directPolyTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
// splitPoint.
void Foam::directHexRef8::insertEdgeSplit
(
...
Foam::label Foam::directHexRef8::storeMidPointInfo
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
...
void Foam::directHexRef8::createInternalFaces
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const labelList& cellMidPoint,
...
void Foam::directHexRef8::walkFaceToMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
void Foam::directHexRef8::walkFaceFromMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
Foam::label Foam::directHexRef8::faceConsistentRefinement
(
    const bool maxSet,
    boolList& refineCell
) const
{
    ...
    for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
    {
        ...
        label ownLevel = cellLevel_[own] + (refineCell[own] ? 1 : 0);

        label nei = mesh_.faceNeighbour()[faceI];
        label neiLevel = cellLevel_[nei] + (refineCell[nei] ? 1 : 0);
    ...
    )
    {
        ...
        neiLevel[faceI] = cellLevel_[own] + (refineCell[own] ? 1 : 0);
    }

    // Swap to neighbour
    syncTools::swapFaceList(mesh_, neiLevel, false);
    ...
    )
    {
        ...
        else if (neiLevel[faceI] > (ownLevel+1))
        {
            if (maxSet)
            {
                refineCell[own] = true;
...
void Foam::directHexRef8::checkWantedRefinementLevels
(
...
) const
{
    ...
    {
        refineCell[cellsToRefine[i]] = true;
    }
    ...
    for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
    {
        ...
        if (mag(ownLevel-neiLevel) > 1)
        {
            FatalErrorIn
            (
                "directHexRef8::checkRefinementLevels(const labelList&)"
            )   << "cell:" << own
    ...
    )
    {
        ...
        if (mag(ownLevel - neiLevel[faceI]) > 1)
        {
...
void Foam::directHexRef8::setInstance(const fileName& inst)
{
    if (debug)
    {
        Pout<< "directHexRef8::setInstance(const fileName& inst) : "
...
Foam::directHexRef8::directHexRef8
(
    const polyMesh& mesh,
    ...
    faceRemover_(mesh_, GREAT)      // merge boundary faces wherever possible
{
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "directHexRef8::directHexRef8(const polyMesh&, const labelList&"
...
Foam::directHexRef8::directHexRef8(const polyMesh& mesh)
:
    mesh_(mesh),
    cellLevel_
    (
    ...
    faceRemover_(mesh_, GREAT)      // merge boundary faces wherever possible
{
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "directHexRef8::directHexRef8(const polyMesh&)"
        )   << "History enabled but number of visible cells in it "
            << history_.visibleCells().size()
            << " is not equal to the number of cells in the mesh "
            << mesh_.nCells()
...
Foam::labelList Foam::directHexRef8::consistentRefinement
(
    const labelList& cellsToRefine,
    const bool maxSet
) const
{
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "directHexRef8::consistentRefinement : Changed " << nChanged
...
Foam::labelList Foam::directHexRef8::consistentSlowRefinement
(
...
) const
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "directHexRef8::consistentSlowRefinement"
    ...
    DynamicList<refinementData> seedFacesInfo(mesh_.nFaces());
    ...
    forAll(facesToCheck, i)
    {
        ...
        if (mesh_.isInternalFace(faceI))
        {
            // Seed face as if neighbouring cell was refined one more than
            // the current owner or neighbour.
    ...
    while(true)
    {
        if (debug)
        {
            Pout<< "directHexRef8::consistentSlowRefinement : Seeded "
    ...
    if (debug)
    {
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            ...
            if (mag(ownLevel-neiLevel) > 1)
            {
                FatalErrorIn
                (
                    "directHexRef8::consistentSlowRefinemet"
                )   << "cell:" << own
                    << " current level:" << cellLevel_[own]
        ...
        labelList neiLevel(mesh_.nFaces());
        labelList neiCount(mesh_.nFaces());
        labelList neiRefCount(mesh_.nFaces());

        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            label nei = mesh_.faceNeighbour()[faceI];
            neiLevel[faceI] = cellLevel_[nei];
            neiCount[faceI] = allCellInfo[nei].count();
            neiRefCount[faceI] = allCellInfo[nei].refinementCount();
        }
        for
        (
            label faceI = mesh_.nInternalFaces();
        ...
        {
            label own = mesh_.faceOwner()[faceI];
            neiLevel[faceI] = cellLevel_[own];
            neiCount[faceI] = allCellInfo[own].count();
            neiRefCount[faceI] = allCellInfo[own].refinementCount();
        }

        // Swap to neighbour
        syncTools::swapFaceList(mesh_, neiLevel, false);
        syncTools::swapFaceList(mesh_, neiCount, false);
        syncTools::swapFaceList(mesh_, neiRefCount, false);
        ...
        )
        {
            ...
            label nbrLevel =
                neiLevel[faceI]
              + ((neiCount[faceI] >= neiRefCount[faceI]) ? 1 : 0);
            ...
            if (mag(ownLevel - nbrLevel) > 1)
            {
                ...
                (
                    ...
                    << " has current level:" << neiLevel[faceI]
                    << " current count:" << neiCount[faceI]
                    << " current refCount:" << neiRefCount[faceI]
    ...
    if (debug)
    {
        Pout<< "directHexRef8::consistentSlowRefinement : From "
...
Foam::labelListList Foam::directHexRef8::setRefinement
(
...
)
{
    if (debug)
    {
        Pout<< "directHexRef8::setRefinement :"
    ...
    forAll(cellLabels, i)
    {
        ...
        (
            ...
            (
                ...
                mesh_.cellPoints()[cellI][0],   // master point
                -1,                             // zone for point
    ...
    if (debug)
    {
        ...
        Pout<< "directHexRef8::setRefinement : Dumping " << splitCells.size()
    ...
    {
        labelList neiLevel(mesh_.nFaces(), labelMax);
        labelList newNeiLevel(mesh_.nFaces(), labelMax);
        ...
        )
        {
            ...
            neiLevel[faceI] = ownLevel;
            newNeiLevel[faceI] = newOwnLevel;
        }

        // Swap.
        syncTools::swapFaceList(mesh_, neiLevel, false);
        syncTools::swapFaceList(mesh_, newNeiLevel, false);
        ...
        )
        {
            ...
            (
                newOwnLevel > faceAnchorLevel[faceI]
             || newNeiLevel[faceI] > faceAnchorLevel[faceI]
            )
            {
                faceMidPoint[faceI] = 12345;    // mark to be split
            }
...

    {
        ...
        // above
        pointField faceMids(mesh_.nFaces(), point(-GREAT, -GREAT, -GREAT));

        forAll(faceMidPoint, faceI)
        {
            if (faceMidPoint[faceI] >= 0)
            {
                faceMids[faceI] = mesh_.faceCentres()[faceI];
            }
        }
        syncTools::syncFaceList
        (
    ...
    if (debug)
    {
        ...
        Pout<< "directHexRef8::setRefinement : Dumping " << splitFaces.size()
    ...
    forAll(cellMidPoint, cellI)
    {
        if (cellMidPoint[cellI] >= 0)
        {
            labelList& cAnchors = cellAnchorPoints[cellI];
            cAnchors.setSize(8);
            label anchorI = 0;
            ...
            forAll(cPoints, i)
            {
                ...
                if (pointLevel_[pointI] <= cellLevel_[cellI])
                {
                    if (anchorI == 8)
                    {
                        FatalErrorIn
                        (
                            "directHexRef8::setRefinement(const labelList&"
                            ", directPolyTopoChange&)"
                        )   << "cell " << cellI
                            << " of level " << cellLevel_[cellI]
                            << " uses more than 8 points of equal or"
                            << " lower level" << nl
                            << "Points so far:" << cAnchors
                            << abort(FatalError);
                    }

                    cAnchors[anchorI++] = pointI;
                }
            }
            cAnchors.setSize(anchorI);
    ...
    if (debug)
    {
        Pout<< "directHexRef8::setRefinement : Splitting faces" << endl;
    }
    ...
    if (debug)
    {
        ...
        {
            FatalErrorIn("directHexRef8::setRefinement")
...
void Foam::directHexRef8::updateMesh(const mapPolyMesh& map)
{
    // Update celllevel
    if (debug)
    {
        Pout<< "directHexRef8::updateMesh :"
...

    {
        ...
        {
            FatalErrorIn("directHexRef8::updateMesh")
    ...
    // Update pointlevel
    {
        ...
        {
            // Assume it is after directHexRef8 that this routine is called.
...
void Foam::directHexRef8::subset
(
...
)
{
    ...
    if (history_.active())
    {
        WarningIn
        (
            "directHexRef8::subset(const labelList&, const labelList&"
            ", const labelList&)"
    ...
    // Update celllevel
    {
        ...
        if (findIndex(cellLevel_, -1) != -1)
        {
            FatalErrorIn("directHexRef8::subset")
                << "Problem : "
                << "cellLevel_ contains illegal value -1 after mapping:"
                << cellLevel_
                << abort(FatalError);
...
void Foam::directHexRef8::distribute(const mapDistributePolyMesh& map)
{
    if (debug)
    {
        Pout<< "directHexRef8::distribute :"
            << " Updating various lists"
            << endl;
    }
...
void Foam::directHexRef8::checkMesh() const
{
    ...
    if (debug)
    {
        Pout<< "directHexRef8::checkMesh : Using matching tolerance smallDim:"
            << smallDim << endl;
    }
    ...
    // cells
    {
        labelList nei(mesh_.faceOwner());
        // Replace data on coupled patches with their neighbour ones.
        syncTools::swapFaceList(mesh_, nei, false);
        ...
        forAll(patches, patchI)
        {
            ...
            if (pp.coupled())
            {
                ...
                forAll(pp, i)
                {
                    ...
                    if (!cellToFace.insert(labelPair(own, nei[faceI]), faceI))
                    {
                        FatalErrorIn("directHexRef8::checkMesh()")
                            << "Faces do not seem to be correct across coupled"
                            << " boundaries" << endl
                            << "Coupled face " << faceI
                            << " between owner " << own
                            << " on patch " << pp.name()
                            << " and coupled neighbour " << nei[faceI]
                            << " has two faces connected to it:"
                            << faceI << " and "
                            << cellToFace[labelPair(own, nei[faceI])]
                            << abort(FatalError);
                    }
    ...
    {
        scalarField neiFaceAreas(mag(mesh_.faceAreas()));

        // Replace data on coupled patches with their neighbour ones.
        syncTools::swapFaceList(mesh_, neiFaceAreas, false);
        ...
        )
        {
            ...
            if (mag(magArea - neiFaceAreas[faceI]) > smallDim)
            {
                label patchI = mesh_.boundaryMesh().whichPatch(faceI);
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    {
        ...
        {
            nVerts[faceI] = mesh_.faces()[faceI].size();
        }

        // Replace data on coupled patches with their neighbour ones.
        syncTools::swapFaceList(mesh_, nVerts, false);
    ...
    {
        // Anchor points.
        pointField anchorPoints(mesh_.nFaces());
        ...
        syncTools::swapFaceList(mesh_, anchorPoints, false);
        ...
        )
        {
            ...
            if (mag(anchorVec - anchorPoints[faceI]) > smallDim)
            {
                label patchI = mesh_.boundaryMesh().whichPatch(faceI);
    ...
    if (debug)
    {
        Pout<< "directHexRef8::checkMesh : Returning" << endl;
    }
}


void Foam::directHexRef8::checkRefinementLevels
(
    const label maxPointDiff,
    const labelList& pointsToCheck
) const
{
    if (debug)
    {
        Pout<< "directHexRef8::checkRefinementLevels :"
    ...
    {
        FatalErrorIn("directHexRef8::checkRefinementLevels(const label)")
    ...
    for (label faceI = mesh_.nInternalFaces(); faceI < mesh_.nFaces(); faceI++)
    {
        ...
        if (mag(cellLevel_[own] - neiLevel[faceI]) > 1)
        {
            label patchI = mesh_.boundaryMesh().whichPatch(faceI);

            FatalErrorIn("directHexRef8::checkRefinementLevels(const label)")
                << "Celllevel does not satisfy 2:1 constraint. On coupled face "
                << faceI
                << " on patch " << patchI << " "
                << mesh_.boundaryMesh()[patchI].name()
        ...
        forAll(pointsToCheck, i)
        {
            ...
            forAll(pCells, i)
            {
                ...
                )
                {
                    FatalErrorIn
                    (
                        "directHexRef8::checkRefinementLevels(const label)"
...
Foam::labelList Foam::directHexRef8::getSplitPoints() const
{
    if (debug)
    {
        checkRefinementLevels(-1, labelList(0));
    ...
    if (debug)
    {
        Pout<< "directHexRef8::getSplitPoints :"
    ...
    if (!history_.active())
    {
        FatalErrorIn("directHexRef8::getSplitPoints()")
...
//void Foam::directHexRef8::markIndex
//(
...
//void Foam::directHexRef8::markCellClusters
//(
//    const label maxLevel,
//    labelList& cluster
//) const
...
Foam::labelList Foam::directHexRef8::consistentUnrefinement
(
...
) const
{
    if (debug)
    {
        Pout<< "directHexRef8::consistentUnrefinement :"
            << " Determining 2:1 consistent unrefinement" << endl;
    }

    if (maxSet)
    {
        FatalErrorIn("directHexRef8::consistentUnrefinement")
    ...
    while (true)
    {
        ...
        {
            label own = mesh_.faceOwner()[faceI];
            label ownLevel = cellLevel_[own] - (unrefineCell[own] ? 1 : 0);

            label nei = mesh_.faceNeighbour()[faceI];
            label neiLevel = cellLevel_[nei] - (unrefineCell[nei] ? 1 : 0);
        ...
        )
        {
            ...
            neiLevel[faceI] = cellLevel_[own] - (unrefineCell[own] ? 1 : 0);
        }
...
void Foam::directHexRef8::setUnrefinement
(
...
)
{
    if (!history_.active())
    {
        FatalErrorIn("directHexRef8::setUnrefinement()")
            << "Only call if constructed with history capability"
            << abort(FatalError);
    }
    ...
    if (debug)
    {
        Pout<< "directHexRef8::setUnrefinement :"
            << " Checking initial mesh just to make sure" << endl;
        ...
        Pout<< "directHexRef8::setRefinement : Dumping " << pSet.size()
            << " points and "
...
bool Foam::directHexRef8::write() const
{
