...
defineTypeNameAndDebug(directCutAndRemove, 0);
...
Foam::label Foam::directCutAndRemove::firstCommon
(
...
bool Foam::directCutAndRemove::isIn
(
    const edge& twoCuts,
...
Foam::label Foam::directCutAndRemove::findCutCell
(
    const cellCuts& cuts,
...
//  none found.
Foam::label Foam::directCutAndRemove::findInternalFacePoint
(
...
) const
{
    ...
    {
        FatalErrorIn("directCutAndRemove::findInternalFacePoint(const labelList&)")
...
// to the patch
Foam::label Foam::directCutAndRemove::findPatchFacePoint
(
    const face& f,
...
void Foam::directCutAndRemove::faceCells
(
    const cellCuts& cuts,
    const label exposedPatchI,
    const label faceI,
...
void Foam::directCutAndRemove::getZoneInfo
(
    const label faceI,
    label& zoneID,
    label& zoneFlip
...
void Foam::directCutAndRemove::addFace
(
...
void Foam::directCutAndRemove::modFace
(
    directPolyTopoChange& meshMod,
    const label faceI,
    const face& newFace,
...
void Foam::directCutAndRemove::copyFace
(
    const face& f,
    const label startFp,
    const label endFp,
...
// (addedPoints_).
void Foam::directCutAndRemove::splitFace
(
    const face& f,
    const label v0,
    const label v1,
...
Foam::face Foam::directCutAndRemove::addEdgeCutsToFace(const label faceI) const
{
    const face& f = mesh().faces()[faceI];
...
Foam::face Foam::directCutAndRemove::loopToFace
(
    const label cellI,
...
Foam::directCutAndRemove::directCutAndRemove(const polyMesh& mesh)
:
    edgeVertex(mesh),
    addedFaces_(),
    addedPoints_()
...
void Foam::directCutAndRemove::setRefinement
(
    const label exposedPatchI,
    const cellCuts& cuts,
...
)
{
    ...
    if (exposedPatchI < 0 || exposedPatchI >= patches.size())
    {
        FatalErrorIn
        (
            "directCutAndRemove::setRefinement("
...
void Foam::directCutAndRemove::updateMesh
(
    ...
    // Update stored labels for mesh change.
    {
        ...
        )
        {
            ...
            if ((newCellI != -1) && (newAddedFaceI != -1))
            {
                ...
                {
                    Pout<< "directCutAndRemove::updateMesh :"
