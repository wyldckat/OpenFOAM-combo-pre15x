...
defineTypeNameAndDebug(directRemoveFaces, 0);
...
void Foam::directRemoveFaces::changeCellRegion
(
    const label cellI,
...
Foam::label Foam::directRemoveFaces::changeFaceRegion
(
...
// - removal of edges
// - removal of points
Foam::boolList Foam::directRemoveFaces::getFacesAffected
(
    const labelList& cellRegion,
...
void Foam::directRemoveFaces::writeOBJ
(
...
{
    OFstream str(fName);
    Pout<< "directRemoveFaces::writeOBJ : Writing faces to file "
        << str.name() << endl;
...
void Foam::directRemoveFaces::mergeFaces
(
    const labelList& cellRegion,
    const labelList& cellRegionMaster,
    const labelHashSet& pointsToRemove,
...
) const
{
    ...
    if (fp.edgeLoops().size() != 1)
    {
        ...
        FatalErrorIn("directRemoveFaces::mergeFaces")
    ...
    forAll(faceLabels, patchFaceI)
    {
        if (patchFaceI != masterIndex)
        {
            ...
            meshMod.setAction(polyRemoveFace(faceLabels[patchFaceI]));
        }
    }
}
...
void Foam::directRemoveFaces::getFaceInfo
(
    const label faceI,
...
Foam::face Foam::directRemoveFaces::filterFace
(
    const labelHashSet& pointsToRemove,
    const label faceI
) const
...
void Foam::directRemoveFaces::modFace
(
    const face& f,
...
// Construct from mesh
Foam::directRemoveFaces::directRemoveFaces
(
    const polyMesh& mesh,
    const scalar minCos
)
...
Foam::label Foam::directRemoveFaces::compatibleRemoves
(
    const labelList& facesToRemove,
    labelList& cellRegion,
...
) const
{
    ...
    forAll(facesToRemove, i)
    {
        ...
        if (!mesh_.isInternalFace(faceI))
        {
            FatalErrorIn
            (
                "directRemoveFaces::compatibleRemoves(const labelList&"
...
void Foam::directRemoveFaces::setRefinement
(
    const labelList& faceLabels,
    const labelList& cellRegion,
    const labelList& cellRegionMaster,
...
) const
{
    ...
    forAll(faceLabels, i)
    {
        ...
        if (!mesh_.isInternalFace(faceI))
        {
            FatalErrorIn
            (
                "directRemoveFaces::setRefinement(const labelList&"
        ...
        forAll(edgeFaces, edgeI)
        {
            if (nFacesPerEdge[edgeI] == -1)
            {
                ...
                else
                {
                    ...
                    FatalErrorIn("directRemoveFaces::setRefinement")
            ...
            if (nFacesPerEdge[edgeI] == 2)
            {
                ...
                if (f0 != -1 && f1 != -1)
                {
                    ...
                    if (patch0 != patch1)
                    {
                        ...
                        WarningIn("directRemoveFaces::setRefinement")
