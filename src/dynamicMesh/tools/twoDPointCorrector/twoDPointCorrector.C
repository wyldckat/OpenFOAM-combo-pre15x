...
void twoDPointCorrector::calcAddressing() const
{
    ...
    forAll (patches, patchI)
    {
        if (typeid(patches[patchI]) == typeid(wedgePolyPatch))
        {
            ...
            if (polyMesh::debug)
            {
                Info << "Found normal from wedge patch " << patchI;
            }
    ...
    if (!isWedge)
    {
        forAll (patches, patchI)
        {
            ...
            )
            {
                ...
                if (polyMesh::debug)
                {
                    Info << "Found normal from empty patch " << patchI;
                }
    ...
    if (polyMesh::debug)
    {
        Info << " twoDPointCorrector normal: " << pn << endl;
    }
...
direction twoDPointCorrector::normalDir() const
{
    ...
    if (pn.x() >= edgeOrthogonalityTol)
    {
        return vector::X;
    }
    else if (pn.y() >= edgeOrthogonalityTol)
    {
        return vector::Y;
    }
    else if (pn.z() >= edgeOrthogonalityTol)
    {
        return vector::Z;
    }
    else
...
void twoDPointCorrector::updateTopology()
{
    if (mesh_.morphing())
    {
        if (polyMesh::debug || polyMesh::morphDebug)
        {
            Info << "twoDPointCorrector::updateTopology(): updating" << endl;
        }
