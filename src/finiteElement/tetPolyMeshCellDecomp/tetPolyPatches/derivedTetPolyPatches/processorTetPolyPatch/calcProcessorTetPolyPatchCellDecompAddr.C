...
void processorTetPolyPatchCellDecomp::calcCutEdgeIndices() const
{
    ...
    const labelList& sharedEdges =
        refCast<const globalProcessorTetPolyPatchCellDecomp>
        (
...
void processorTetPolyPatchCellDecomp::calcOwnNeiDoubleMask() const
{
    ...
    const unallocLabelList& L = mesh.ldu().lowerAddr();
    const unallocLabelList& U = mesh.ldu().upperAddr();
    ...
    if (!localEdgeIndicesPtr_)
    {
        if (isMaster())
        {
            localEdgeIndicesPtr_ =
                new labelList(calcProcLocalEdgesIndices(procPatch()));
        }
        else
        {
