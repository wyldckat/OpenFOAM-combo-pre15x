...
void Foam::tetPolyMeshCellDecomp::addParallelPointPatch()
{
    ...
    if (mesh_.parallelData().parallel())
    {
        ...
        (
            ...
            (
                "pointProcAddressing",
                mesh_.time().findInstance(operator()().meshDir(), "cells"),
                mesh_.meshSubDir,
                mesh_,
                IOobject::MUST_READ,
                IOobject::NO_WRITE
        ...
        const label globalCellOffset = mesh_.parallelData().nTotalPoints();
        ...
        const labelList& localParPoints =
          mesh_.parallelData().sharedPointLabels();
        ...
        const unallocLabelList& L = ldu().lowerAddr();
        const unallocLabelList& U = ldu().upperAddr();
        ...
        {
            Info<< "void tetPolyMeshCellDecomp::addParallelPointPatch() : "
                << "Adding globalProcessorTetPolyPatchCellDecomp."
                << "  nGlobalPoints: "
                << mesh_.parallelData().nGlobalPoints() << endl;
        }

        // Hook the processor point patch
        boundary_.setSize(boundary_.size() + 1);

        boundary_.hook
        (
            new globalProcessorTetPolyPatchCellDecomp
            (
                mesh_.parallelData().nGlobalPoints(),
                localParPoints,
                mesh_.parallelData().sharedPointAddr(),
                globalParEdges.size(),
                localParEdges,
                localEdgeMapping,
                localCutEdges,
