...
)
{
    ...
    tetFemMatrix<Type>& fvm = tfvm();

    // Get reference to upper and diagonal
    scalarField& u = fvm.upper();
    ...
    const lduAddressing& lduAddr = fvm.lduAddr();
    ...
    for (label cellI = 0; cellI < mesh.nCells(); cellI++)
    {
        ...
        for (label localI = 0; localI < nCellPoints; localI++)
        {
            ...
            for
            (
                label globajJEdge = startLabel;
                globajJEdge < endLabel;
                globajJEdge++
            )
            {
                label localJ = globalToLocalBuffer[neighbour[globajJEdge]];
...
)
{
    ...
    const Field<Type>& psi = fvm.psi().internalField();

    // Get reference to upper and diagonal
    scalarField& u = fvm.upper();
    scalarField& d = fvm.diag();
    vectorField& source = fvm.source();
    ...
    for (label cellI = 0; cellI < mesh.nCells(); cellI++)
    {
        ...
        for (label localI = 0; localI < nCellPoints; localI++)
        {
            ...
            {
                label globalJ = neighbour[globajJEdge];
                ...
                if (localJ > -1)
                {
                    ...
                    u[globajJEdge] += curGamma*tr(curOffDiagCoeff);
