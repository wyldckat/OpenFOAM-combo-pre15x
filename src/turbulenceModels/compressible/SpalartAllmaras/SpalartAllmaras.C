...
tmp<volSymmTensorField> SpalartAllmaras::R() const
{
    ...
    (
        ...
        (
            ...
            ((2.0/3.0)*I)*k() - (mut_/rho_)*2*dev(symm(fvc::grad(U_)))
        )
    );
}


tmp<fvVectorMatrix> SpalartAllmaras::divRhoR(volVectorField& U) const
{
...
void SpalartAllmaras::enforceNoWrite()
{
	mut_.writeOpt() = IOobject::NO_WRITE;
	nuTilda_.writeOpt() = IOobject::NO_WRITE;
}
