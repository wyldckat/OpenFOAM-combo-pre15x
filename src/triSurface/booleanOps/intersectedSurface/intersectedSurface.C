...
)
{
    ...
    forAll(edgeLabels, i)
    {
        ...
        if (stat == UNVISITED)
        {
            Info<< "    edge:" << edgeI << "  verts:" << e
                << "  unvisited" << nl;
        }
        else if (stat == STARTTOEND)
        {
            Info<< "    edge:" << edgeI << "  from " << e[0]
                << " to " << e[1] << nl;
        }
        else if (stat == ENDTOSTART)
        {
            Info<< "    edge:" << edgeI << "  from " << e[1]
                << " to " << e[0] << nl;
        }
        else
        {
            Info<< "    edge:" << edgeI << "  both " << e
                << nl;
        }
    }
    Info<< endl;
...
)
{
    ...
    if (debug & 2)
    {
        ...
        Info<< "calcPointEdgeAddressing: face consisting of edges:" << endl;
        forAll(fEdges, i)
        {
            label edgeI = fEdges[i];
            const edge& e = edges[edgeI];
            Info<< "    " << edgeI << ' ' << e << points[e.start()]
                << points[e.end()] << endl;
        }

        Info<< "    Constructed point-edge adressing:" << endl;
        for
        (
            Map<DynamicList<label> >::iterator iter = facePointEdges.begin();
            iter != facePointEdges.end();
        ...
        {
            Info<< "    vertex " << iter.key() << " is connected to edges "
                << iter() << endl;
        }
        Info<<endl;
    }
...
)
{
    ...
    if (connectedEdges.size() <= 1)
    {
        ...
        {
            Info<< "Writing face:" << faceI << " to face.obj" << endl;
            OFstream str("face.obj");
            writeOBJ(points, edges, fEdges, str);

            Info<< "Writing connectedEdges edges to faceEdges.obj" << endl;
    ...
    if (connectedEdges.size() == 2)
    {
        ...
        if (connectedEdges[0] == prevEdgeI)
        {
            if (debug & 2)
            {
                Info<< "Stepped from edge:" << edges[prevEdgeI]
...
)
{
...

    {
        forAllConstIter(Map<label>, pointVisited, iter)
        {
            ...
            Info<< "point:" << pointI << "  nVisited:" << nVisits
    ...
    Info<< "resplitFace : adding intersection from " << visitedVert0
...
//    additionalEdges[1] = edge(visitedVert1, unvisitedVert1);
    ...
    Info<< "Dumping face:" << faceI << " to " << newFName << endl;
    writeLocalOBJ
    (
        eSurf.points(),
        eSurf.edges(),
...
)
{
    ...
    while (true)
    {
        ...
        //Info<< "splitFace: starting walk from edge:" << startEdgeI
    ...
    forAll(fEdges, i)
    {
        ...
        {
            //{
            //    Info<< "Dumping faces so far to faces.obj" << nl
            //        << faces << endl;
            //
...
//                      + ".obj"
//                    );
//                    Info<< "Writing original face:" << faceI << " subFace:"
