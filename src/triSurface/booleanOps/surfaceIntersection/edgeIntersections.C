...
Foam::scalar Foam::edgeIntersections::alignedCos_ =
    Foam::cos(89.0 * Foam::physicalConstant::pi/180.0);
...
)
{
    ...
    if (debug)
    {
        Info<< "Calculating intersection of " << edgeLabels.size() << " edges"
    ...
    forAll(edgeLabels, i)
    {
        ...
        if (debug && (i % 1000 == 0))
        {
            Info<< "Intersecting edge " << edgeI << " with surface" << endl;
        }
    ...
    if (debug)
    {
        Info<< "Found " << nHits << " intersections of edges with surface ..."
            << endl;
    }
...
) const
{
    ...
    forAll(edgeEnds, i)
    {
        if (edgeEnds[i] == 2)
        {
            ...
            Info<< "rotating: shifting endpoint " << meshPoints[pointI]
                << " of edge:" << edgeI << " verts:"
