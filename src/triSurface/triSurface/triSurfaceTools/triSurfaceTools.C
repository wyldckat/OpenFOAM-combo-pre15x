...
// Get all edges of triangle in face ordering.
Foam::FixedList<Foam::label, 3> Foam::triSurfaceTools::sortedFaceEdges
(
    const triSurface& surf,
    const label faceI
)
...
{
    FixedList<label, 3> myEdges =
        Foam::triSurfaceTools::sortedFaceEdges
        (
            surf,
            faceI
        );
    ...
    else if (edgeI == myEdges[1])
    {
        e1 = myEdges[2];
        e2 = myEdges[0];
    }
    else if (edgeI == myEdges[2])
    {
        e1 = myEdges[0];
        e2 = myEdges[1];
    }
...
////  Radius of circumscribed circle. Graphics Gems I p22
//Foam::scalar Foam::triSurfaceTools::circumRadius
//(
//    const point& pa,
//    const point& pb,
//    const point& pc
//)
//{
//    const vector pca = pc - pa;
//    const vector pba = pb - pa;
//    const vector pcb = pc - pb;
//
//    scalar d1 = pca & pba;
//    scalar d2 = - pcb & pba;
//    scalar d3 = pca & pcb;
...
//// Simple quality of triangle check. Ratio of area v.s. area of circumcircle.
//// normalized to 1(equilateral) .. 0(sliver)
//Foam::scalar Foam::triSurfaceTools::triQuality
//(
//    const point& pa,
//    const point& pb,
//    const point& pc
...
//    vector e1(pc - pa);
//
//    scalar area = 0.5*Foam::mag(e0 ^ e1);
//
//    scalar radius = circumRadius(pa, pb, pc);
//
//    return area/(M_PI*Foam::sqr(radius) * 0.413497);
//}
...
Foam::scalar Foam::triSurfaceTools::pseudoAngle
(
    const vector& e0,
    const vector& e1,
    const vector& vec
)
{
    ...
    if (sin < -SMALL)
    {
        return 3.0*M_PI/2 + cos*M_PI/2;
    }
    else
    {
        return M_PI/2 - cos*M_PI/2;
    }
}
...
//// Is when the triangles neighbouring the collapsed one already share a vertex.
...
//    }
//
//    // Now neighbours contains first layer of triangles outside of collapseFaces
//    // There should be 
...
)
{
    ...
    // Get vertices in ordering consistent with edges

    label splitI = -1;
    label nextI = -1;
    label nextNextI = -1;

    if (otherEdges[0] == edgeI)
    {
        splitI = otherEdges[0];
        nextI = otherEdges[1];
        nextNextI = otherEdges[2];
    }
    else if (otherEdges[1] == edgeI)
    {
        splitI = otherEdges[1];
        nextI = otherEdges[2];
        nextNextI = otherEdges[0];
    }
    else
    {
        splitI = otherEdges[2];
        nextI = otherEdges[0];
        nextNextI = otherEdges[1];
    }

    label splitVert0 =
            edges[splitI].commonVertex(edges[nextNextI]);
    label splitVert1 =
            edges[splitI].commonVertex(edges[nextI]);
    label farVertI =
            edges[nextI].commonVertex(edges[nextNextI]);
    ...
    forAll(refineStatus, faceI)
    {
        if (refineStatus[faceI] == RED)
        {
            ...
            (
                ...
                (
                    edgeMid[myEdges[0]],
                    edges[myEdges[0]].commonVertex(edges[myEdges[1]]),
                    edgeMid[myEdges[1]],
                    surf[faceI].region()
                )
            );
            ...
            (
                ...
                (
                    edgeMid[myEdges[1]],
                    edges[myEdges[1]].commonVertex(edges[myEdges[2]]),
                    edgeMid[myEdges[2]],
                    surf[faceI].region()
                )
            );
            ...
            (
                ...
                (
                    edgeMid[myEdges[2]],
                    edges[myEdges[2]].commonVertex(edges[myEdges[0]]),
                    edgeMid[myEdges[0]],
                    surf[faceI].region()
                )
            );
            ...
            forAll(myEdges, myEdgeI)
            {
                const label edgeI = myEdges[myEdgeI];
...
{
    labelList refineStatus(surf.size(), NONE);
...
)
{
    ...
    // Get a typical length scale
    scalar typDim =
        (1.0/3.0)
      * (
            mag(localPts[localF[0]] - localPts[localF[1]])
          + mag(localPts[localF[1]] - localPts[localF[2]])
          + mag(localPts[localF[2]] - localPts[localF[0]])
        )
      + VSMALL;

    scalar tolDim = typDim*intersection::planarTol();
    ...
    (
        ...
        localPts[localF[1]],
        localPts[localF[2]]
    ).classify(nearestPt, tolDim, nearType, nearLabel);
    ...
    else if (nearType == triPointRef::EDGE)
    {
        ...
        edge nearEdge(localF[nearLabel], localF[(nearLabel + 1) % 3]);

        // Find edge label
        const labelList& myEdges = surf.faceEdges()[nearestFaceI];

        label edgeI = -1;

        forAll(myEdges, myEdgeI)
        {
            if (surf.edges()[myEdges[myEdgeI]] == nearEdge)
            {
                edgeI = myEdges[myEdgeI];
        ...
        forAll(myFaces, myFaceI)
        {
            edgeNormal += surf.faceNormals()[myFaces[myFaceI]];
        }
