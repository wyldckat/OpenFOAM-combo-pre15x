...
) const
{
    ...
    if (debug)
    {
        Info<< "orientedSurface::propagateOrientation : starting face"
            << " orientation:" << nl
            << "     for outsidePoint:" << outsidePoint << nl
            << "     starting from point:" << nearestPt << nl
            << "     on face:" << nearestFaceI << nl
            << "     with normal:" << n << endl;
    }

    if (((outsidePoint - nearestPt) & n) > 0)
    {
...
void Foam::orientedSurface::orientSurface(const point& outsidePoint)
{
...
:
    triSurface(surf)
{
    orientSurface(outsidePoint);
}


// Construct from surface. Calculate outside point.
Foam::orientedSurface::orientedSurface(const triSurface& surf)
:
    triSurface(surf)
{
