...
template<class Type>
inline void limitedGrad<Type>::limitFace
(
    scalar& limiter,
    ...
    forAll(bvf, patchi)
    {
        ...
        if (psf.coupled())
        {
            ...
            forAll(pOwner, facei)
            {
                ...
                vector vvfOwn = vvf[own];
                vector vvfNei = psfNei[facei];

                vector gradf = (pCf[facei] - C[own]) & g[own];
