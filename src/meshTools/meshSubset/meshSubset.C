...
void Foam::meshSubset::doCoupledPatches(labelList& nCellsUsingFace) const
{
...
void Foam::meshSubset::setLargeCellSubset
(
...
)
{
    ...
    if (region.size() != oldCells.size())
    {
        FatalErrorIn
        (
            "meshSubset::setCellSubset(const labelList&"
    ...
    forAll (oldFaces, oldFaceI)
    {
        ...
        if (region[oldAllOwner[oldFaceI]] == currentRegion)
        {
    ...
    for (label faceI = nInternalFaces; faceI < faceMap_.size(); faceI++)
    {
        ...
        if (isInternalFace(oldFaceI))
        {
            ...
            if
            (
                region[oldAllOwner[oldFaceI]] != currentRegion
             && region[oldNeighbour[oldFaceI]] == currentRegion
            )
            {
                oldF = oldFaces[oldFaceI].reverseFace();
...
{
    labelList region(allCells().size(), 0);
