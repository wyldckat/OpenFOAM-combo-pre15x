...
{
    Info<< "    Field min:" << min(field).value()
...
) const
{
    ...
    if (!fieldObject.headerOk())
    {
        ...
        (
            ...
            << " from time " << fMesh.time().timeName() << exit(FatalError);
    }

    if (fieldObject.headerClassName() == volScalarField::typeName)
    {
        volScalarField field(fieldObject, fMesh);

        applyToSet(action, field, set);
    }
    else if (fieldObject.headerClassName() == volVectorField::typeName)
    {
        volVectorField field(fieldObject, fMesh);

        applyToSet(action, Foam::mag(field), set);
    }
    else
    {
        FatalErrorIn
