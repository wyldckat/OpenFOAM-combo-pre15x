...
)
{
    ...
    const label startFaceOwn =
        matrix.psi().mesh().ldu().ownerStartAddr()[rowID_];
    const label endFaceOwn =
        matrix.psi().mesh().ldu().ownerStartAddr()[rowID_ + 1];

    const label startFaceNbr =
        matrix.psi().mesh().ldu().losortStartAddr()[rowID_];
    const label endFaceNbr =
        matrix.psi().mesh().ldu().losortStartAddr()[rowID_ + 1];

    const unallocLabelList& losort = matrix.psi().mesh().ldu().losortAddr();
...
)
{
    ...
    const label startFaceOwn =
        matrix.psi().mesh().ldu().ownerStartAddr()[rowID];
    const label endFaceOwn =
        matrix.psi().mesh().ldu().ownerStartAddr()[rowID + 1];

    const label startFaceNbr =
        matrix.psi().mesh().ldu().losortStartAddr()[rowID];
    const label endFaceNbr =
        matrix.psi().mesh().ldu().losortStartAddr()[rowID + 1];

    const unallocLabelList& owner = matrix.psi().mesh().ldu().lowerAddr();
    const unallocLabelList& neighbour = matrix.psi().mesh().ldu().upperAddr();
    const unallocLabelList& losort = matrix.psi().mesh().ldu().losortAddr();
...
) const
{
    ...
    if (fcOfD > SMALL)
    {
        ...
        const unallocLabelList& owner =
            matrix.psi().mesh().ldu().lowerAddr();
        const unallocLabelList& neighbour =
            matrix.psi().mesh().ldu().upperAddr();
        const unallocLabelList& losort =
            matrix.psi().mesh().ldu().losortAddr();
...
) const
{
    ...
    if (componentOfValue(fc, d) > SMALL)
    {
        ...
        psiCmpt[rowID()] = componentOfValue(fc, d)*componentOfValue(value(), d);
    }
}
