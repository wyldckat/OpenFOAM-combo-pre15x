...
gzfilebuf *gzfilebuf::open(const char *name, int io_mode)
{
    ...
    else if (io_mode & std::ios_base::app)
    {
    ...
    if (io_mode & std::ios_base::binary)
    {
    ...
    if (io_mode & (std::ios_base::out|std::ios_base::app))
    {
        *p++ = '9';
    }
...
std::streampos gzfilebuf::seekoff
(
    std::streamoff,
