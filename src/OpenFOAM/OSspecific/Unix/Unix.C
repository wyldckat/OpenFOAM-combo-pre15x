...
fileName Foam::dotFoam(const fileName& name)
{
    ...
    if (exists(localDotFoamFileName))
    {
        return localDotFoamFileName;
    }

    if (!env("WM_PROJECT_DIR"))
    {
        cerr<< "dotFoam(const fileName&) : "
            << "Couldn't find environment variable WM_PROJECT_DIR"
            << std::endl << std::endl;
        ::exit(1);
    }

    fileName dotFoamFileName = getEnv("WM_PROJECT_DIR")/dotDir/name;

    if (exists(dotFoamFileName))
    {
        return dotFoamFileName;
    }

    cerr<< "dotFoam(const fileName&) : "
        << "Couldn't find file " << localDotFoamFileName.c_str()
        << " or " << dotFoamFileName.c_str()
        << std::endl << std::endl;
    ::exit(1);
...
fileName::Type Foam::type(const fileName& name)
{
    mode_t m = mode(name);
