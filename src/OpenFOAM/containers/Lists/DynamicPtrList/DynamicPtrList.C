...
#include "DynamicPtrList.H"

// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //

template<class T>
Foam::label Foam::DynamicPtrList<T>::sizeIncrement_ = 0;

template<class T>
Foam::label Foam::DynamicPtrList<T>::sizeMultiplier_ = 2;
...
(
    Foam::Ostream& os,
    const Foam::DynamicPtrList<T>& DL
)
{
    ((DynamicPtrList<T>&)DL).setSize(DL.nextFree());
    os << (const List<T>&)DL;
    return os;
}
