...
void pressureTransmissiveFvPatchScalarField::updateCoeffs()
{
    ...
    word ddtScheme(patchMesh().boundaryMesh().mesh().ddtScheme("p"));
    ...
    scalarField w = (patchMesh().nf() & Up) + sqrt(gamma/psip);
    ...
    Field<scalar> alpha = w*ldb.deltaT().value()*patchMesh().deltaCoeffs();
