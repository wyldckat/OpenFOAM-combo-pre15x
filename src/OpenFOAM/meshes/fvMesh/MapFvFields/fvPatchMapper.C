...
void Foam::fvPatchMapper::calcAddressing() const
{
    ...
    const label oldPatchStart =
        patch_.boundaryMesh().mesh().morphMap().oldPatchStarts()
        [patch_.index()];

    const label oldPatchEnd =
        oldPatchStart
      + patch_.boundaryMesh().mesh().morphMap().oldPatchSizes()
        [patch_.index()];
...
:
    patch_(patch),
    faceMap_(faceMap),
    resizeOnly_(resizeOnly),
    sizeBeforeMapping_
    (
        patch_.boundaryMesh().mesh().morphMap().oldPatchSizes()[patch_.index()]
    ),
    directPtr_(NULL),
    directAddrPtr_(NULL),
