...
void polyMesh::zipUpCells()
{
    if (debug)
    {
        Info<< "bool polyMesh::zipUpCells() const: "
...
#                           endif
        ...
        // Re-do face zones with the new face list
        List<faceZone*> newFaceZones(faceZones_.size());

        // Renumber the face zones
        forAll (faceZones_, pzI)
        {
            newFaceZones[pzI] =
                faceZones_[pzI].clone
                (
                    faceZones_,
                    pzI,
                    faceZones_[pzI].addressing(),
                    faceZones_[pzI].flipMap()
                ).ptr();
        }

        // Re-hook the face zones
        faceZones_.clear();
        
        forAll (newFaceZones, pzI)
        {
            faceZones_.hook(newFaceZones[pzI]);
        }
