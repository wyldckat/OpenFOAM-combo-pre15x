...
template <class Type>
const Foam::scalar Foam::meshWave<Type>::geomTol_ = 1e-6;

template <class Type>
const Foam::scalar Foam::meshWave<Type>::propagationTol_ = 0.01;

// Write to ostream
template <class Type>
Foam::Ostream& Foam::meshWave<Type>::writeFaces
(
    const label nFaces,
...
// Read from istream
template <class Type>
Foam::Istream& Foam::meshWave<Type>::readFaces
(
    label& nFaces,
...
template <class Type>
bool Foam::meshWave<Type>::updateCell
(
    const label cellI,
...
template <class Type>
bool Foam::meshWave<Type>::updateFace
(
    const label faceI,
...
template <class Type>
void Foam::meshWave<Type>::checkCyclic(const polyPatch& patch) const
{
    ...
    for(label patchFaceI = 0; patchFaceI < cycOffset; patchFaceI++)
    {
        ...
        if (!allFaceInfo_[i1].sameGeometry(allFaceInfo_[i2], geomTol_))
        {
            Pout<< procLabel_.c_str()
                ...
                << endl;
            FatalErrorIn("meshWave<Type>::checkCyclic(const polyPatch&)")
                << abort(FatalError);
        }
...
template <class Type>
bool Foam::meshWave<Type>::hasPatchType(const word& nameOfType)
{
...
template <class Type>
void Foam::meshWave<Type>::setFaceInfo
(
...
template <class Type>
void Foam::meshWave<Type>::mergeFaceInfo
(
...
template <class Type>
Foam::label Foam::meshWave<Type>::getChangedPatchFaces
(
    const polyPatch& patch,
...
template <class Type>
void Foam::meshWave<Type>::leaveDomain
(
    const polyPatch& patch,
    const label nFaces,
    const labelList& faceLabels,
...
) const
{
    ...
    for(label i = 0; i < nFaces; i++)
    {
        ...
        label meshFaceI = patch.start() + patchFaceI;
        faceInfo[i].leaveDomain(patch, patchFaceI, faceCentres[meshFaceI]);
    }
}
...
template <class Type>
void Foam::meshWave<Type>::enterDomain
(
    const polyPatch& patch,
    const label nFaces,
    const labelList& faceLabels,
...
) const
{
    ...
    for(label i = 0; i < nFaces; i++)
    {
        ...
        label meshFaceI = patch.start() + patchFaceI;
        faceInfo[i].enterDomain(patch, patchFaceI, faceCentres[meshFaceI]);
    }
}
...
template <class Type>
void Foam::meshWave<Type>::transform
(
...
)
{
    ...
    else
    {
        for(label faceI = 0; faceI < nFaces; faceI++)
        {
            faceInfo[faceI].transform(rotTensor[faceI]);
        }
    }
}
...
template <class Type>
void Foam::meshWave<Type>::sendPatchInfo
(
...
template <class Type>
Foam::label Foam::meshWave<Type>::receivePatchInfo
(
    const label neighbour,
    labelList& faceLabels,
    List<Type>& faceInfo
...
template <class Type>
void Foam::meshWave<Type>::offset
(
    const polyPatch&,
...
template <class Type>
void Foam::meshWave<Type>::handleProcPatches()
{
    // Send all
...
template <class Type>
void Foam::meshWave<Type>::handleCyclicPatches()
{
    forAll(mesh_.boundaryMesh(), patchI)
    {
        const polyPatch& patch = mesh_.boundaryMesh()[patchI];
...
template <class Type>
Foam::label Foam::meshWave<Type>::getUnsetCells() const
{
...
template <class Type>
Foam::label Foam::meshWave<Type>::getUnsetFaces() const
{
...
template <class Type>
Foam::label Foam::meshWave<Type>::faceToCell()
{
    ...
    )
    {
        ...
        if (!changedFace_[faceI])
        {
            FatalErrorIn("meshWave<Type>::faceToCell()")
                << "Face " << faceI
...
// Propagate cell to face
template <class Type>
Foam::label Foam::meshWave<Type>::cellToFace()
{
    ...
    )
    {
        ...
        if (!changedCell_[cellI])
        {
            FatalErrorIn("meshWave<Type>::cellToFace()") << "Cell " << cellI
                << " not marked as having been changed"
                << abort(FatalError);
        }
...
// Iterate
template <class Type>
Foam::label Foam::meshWave<Type>::iterate(const label maxIter)
{
    while(iter_ < maxIter)
    {
        if (debug)
