...
template<class Type>
void faMatrix<Type>::addBoundarySource(Field<Type>& source) const
{
...
#   endif
    ...
    if (fam.faceFluxCorrectionPtr_)
    {
        ...
        GeometricField<Type, faPatchField, edgeMesh>
        (
...
template<class Type>
bool faMatrix<Type>::needReference()
{
    ...
    if (!searched)
    {
        const typename GeometricField<Type, faPatchField, areaMesh>::
            GeometricBoundaryField& patchFields = psi_.boundaryField();
...
void faMatrix<Type>::relax(const scalar alpha)
{
    ...
    forAll(psi_.boundaryField(), patchI)
    {
        ...
        if (ptf.coupled())
        {
            scalarField diagAdd = cmptAv
            (
                max(internalCoeffs_[patchI], cmptMag(boundaryCoeffs_[patchI]))
            ...
            (
                lduAddr().patchAddr(patchI),
                diagAdd*ptf.patchInternalField(),
                source()
            );
        }
        else
...
template<class Type>
tmp<GeometricField<Type, faPatchField, edgeMesh> > faMatrix<Type>::
flux() const
{
    ...
    // construct GeometricField<Type, faPatchField, edgeMesh>
    tmp<GeometricField<Type, faPatchField, edgeMesh> > tfieldFlux
    (
    ...
    GeometricField<Type, faPatchField, edgeMesh>& fieldFlux = tfieldFlux();
...
template<class Type>
tmp<Field<Type> > faMatrix<Type>::operator&
(
...
) const
{
    ...
    // 
    FieldField<Field, scalar> coupledBouCoeffs(0);
    lduCoupledInterfacePtrsList interfaceFields(0);
...
template<class Type>
lduMatrix::solverPerformance solve(faMatrix<Type>& fam)
{
    return fam.solve();
}
