...
void faMesh::calcLduAddressing() const
{
    ...
    lduPtr_ = new lduAddressingFaMesh(*this);
}
...
void faMesh::calcLe() const
{
    ...
    {
        const labelList::subList bndEdgeFaces =
...
void faMesh::calcS() const
{
    ...
    SPtr_ = new scalarField(nFaces(), 0.0);
    scalarField& S = *SPtr_;
...
void faMesh::calcEdgeTransformTensors() const
{
    ...
    {
        edgeTransformTensors.hook(new Field<tensor>(3, I));
    ...
    forAll (boundary(), patchI)
    {
        const labelList::subList edgeFaces = boundary()[patchI].edgeFaces();

        forAll(edgeFaces, edgeI)
        {
            edgeTransformTensors.hook(new Field<tensor>(2, I));
