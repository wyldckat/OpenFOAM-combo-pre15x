...
#   endif
    ...
    GeometricField<Type, faPatchField, edgeMesh>& sf = tsf();
    ...
    {
        const faPatchScalarField& pLambda = lambdas.boundaryField()[pi];
        const faPatchScalarField& pY = ys.boundaryField()[pi];
        ...
        if (vf.boundaryField()[pi].coupled())
        {
            ...
            (
                ...
                "const tmp<edgeScalarField>&, "
                "const tmp<edgeScalarField>&) "
            )   << "Interpolation is not implemented for coupled patches!"
                << abort(FatalError);
...
{
    tmp<GeometricField<Type, faPatchField, edgeMesh> > tinterpVf
        = interpolate(tvf());
    tvf.clear();
    return tinterpVf;
}
